...
#define DEFINE_JSON_PARSE_BASE_TYPE( T, CJSON_TYPE, CJSON_VALUE ) \
template< class N = T>\
static bool ParseBasicType( T & _v, cJSON * _json)\
{\
if (!_json) return false; \
if (_json->type != CJSON_TYPE)  return false;\
	_v = (T)_json->CJSON_VALUE; \
	return true; \
}\
\
template<class N = T>\
static bool ParseBasicType(std::vector< T >& _v, cJSON * _json)\
{\
if (!_json) return false; \
...
if (_json->type != CJSON_TYPE)  return false;\
_v.push_back((T)item->CJSON_VALUE); \
}\
return true; \
}\
template<>static bool ParseObject( T & _value, cJSON* _json) { return false; }\
template<>static bool ParseObject( std::vector< T >& _value, cJSON* _json) { return false; }\
...
#define DEFINE_JSON_SAVE_NUMBER(T) \
template<> static bool SaveObject( T & _value, cJSON* _json, const char * _key)\
{\
	return false;\
}\
template<> static bool SaveObject(std::vector< T >& _value, cJSON* _json, const char * _key)\
{\
	return false;\
}\
\
template <> static bool SaveBasicType(T & _value, cJSON* _json, const char * _key)\
{\
	cJSON_AddNumberToObject(_json, _key, (double)_value);\
	return true;\
}\
\
template<> static bool SaveBasicType(std::vector< T >& _value, cJSON* _json, const char * _key)\
{\
...
template <> static bool SaveBasicType(std::string& _value, cJSON* _json, const char * _key)\
{\
	cJSON_AddStringToObject(_json, _key, _value.c_str());\
	return true;\
}\
template <> static bool SaveBasicType(std::vector< std::string >& _value, cJSON* _json, const char * _key)\
{\
	cJSON* arr = cJSON_CreateArray();\
	cJSON_AddItemToObject(_json, _key, arr);\
	for (auto& v : _value)\
...
template<> static bool SaveObject(std::string& _value, cJSON* _json, const char * _key)\
{\
	return false;\
}\
template<> static bool SaveObject(std::vector< std::string >& _value, cJSON* _json, const char * _key)\
{\
	return false;\
}\
...
template <> static bool SaveBasicType(bool& _value, cJSON* _json, const char * _key)\
{\
	cJSON_AddBoolToObject(_json, _key, _value);\
	return true;\
}\
template <> static bool SaveBasicType(std::vector<bool>& _value, cJSON* _json, const char * _key)\
{\
	cJSON* arr = cJSON_CreateArray();\
	cJSON_AddItemToObject(_json, _key, arr);\
	for (auto&& v : _value)\
...
template<> static bool SaveObject(bool& _value, cJSON* _json, const char * _key)\
{\
	return false;\
}\
template<> static bool SaveObject(std::vector<bool>& _value, cJSON* _json, const char * _key)\
{\
	return false;\
}\
